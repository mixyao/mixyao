...
public class MyCustomClipEditor : ClipEditor
{
    public override ClipDrawOptions GetClipOptions(TimelineClip clip)
    {
        ...
        MyCustomClip myClip = clip.asset as MyCustomClip;

        if (myClip != null && myClip.clipSettings != null)
        {
            Texture2D icon = myClip.clipSettings.icon != null ? myClip.clipSettings.icon : Resources.Load<Texture2D>(myClip.clipSettings.iconPath);
            if (icon != null)
            {
    ...
    public override void DrawBackground(TimelineClip clip, ClipBackgroundRegion region)
    {
        ...
        if (myClip != null && myClip.clipSettings != null)
        {
            ...
            RenderBackGroundIcon(myClip.clipSettings.icon, myClip.clipSettings.size, myClip.clipSettings.iconPath, region);
        }
    }
